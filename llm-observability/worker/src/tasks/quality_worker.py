    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                SELECT prompt_version, AVG(score) AS avg_score
                FROM recent
                WHERE rn <= %s
            ...
            for prompt_version, avg_score in cur.fetchall():
                alerts += 1
                logger.warning(
